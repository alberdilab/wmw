...
PACKAGE_INIT_PATH = ROOT / "src" / "wmw" / "__init__.py"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare a wmw release by cutting the CHANGELOG entry from "
    ...
    parser.add_argument(
        "version",
        help="Release version to write into pyproject.toml, src/wmw/__init__.py, and CHANGELOG.md.",
    )
    parser.add_argument(
        "--date",
        dest="release_date",
...
def update_package_version(content: str, version: str) -> str:
    ...
    if count != 1:
        raise ReleaseError("Could not find a unique __version__ line in src/wmw/__init__.py.")
    return updated
...
def run_command(command: list[str], *, shell: bool = False) -> None:
    ...
    subprocess.run(rendered if shell else command, cwd=ROOT, shell=shell, check=True)


def print_release_plan(plan: ReleasePlan, changed_files: list[str]) -> None:
    print("wmw release plan")
